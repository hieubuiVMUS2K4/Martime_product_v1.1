...
import { MaintenanceSchedule } from '../../types/pms.types';
...
export const MasterSchedulePage: React.FC = () => {
  ...
  const getSchedulePosition = (schedule: MaintenanceSchedule) => {
    ...
    const dueDate = parseISO(schedule.nextDueAt);
    const index = days.findIndex(day => isSameDay(day, dueDate));
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `master-schedule-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    a.click();
  };

  const days = getDaysInView();
  const filteredSchedules = schedules.filter(s => s.nextDueAt);
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="text-2xl font-bold text-gray-900 mt-1">
            {schedules.reduce((sum, s) => sum + s.estimatedManHours, 0)}
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="text-sm text-gray-600">Critical Tasks</div>
          <div className="text-2xl font-bold text-red-600 mt-1">
            {schedules.filter(s => s.equipmentName?.includes('CRITICAL')).length}
          </div>
        </div>
      </div>
                ...
                return (
                  <div key={schedule.id} className="flex hover:bg-gray-50">
                    ...
                    <div className="w-80 flex-shrink-0 px-4 py-3 border-r border-gray-200">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-0.5 rounded ${getMaintenanceTypeColor(schedule.maintenanceType)}`}>
                          {schedule.maintenanceType}
                        </span>
                        <span className="text-xs text-gray-500">
                          {schedule.estimatedManHours}h
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 relative py-3">
                      ...
                      <div
                        ...
                        title={`Due: ${schedule.nextDueAt ? format(parseISO(schedule.nextDueAt), 'MMM d, yyyy') : 'N/A'}`}
                      >
