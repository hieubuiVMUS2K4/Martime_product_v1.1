...
WITH expanded AS (
    SELECT
        ...
        format('%s BENCH-smoke-%s', selected.full_name, lpad(gs::text, 6, '0')) AS bench_name,
        NOW() + make_interval(secs => (gs % 300)) + make_interval(msecs => gs) AS bench_updated_at
