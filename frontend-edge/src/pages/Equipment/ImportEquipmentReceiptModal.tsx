...
import { equipmentReceiptService, type ImportEquipmentReceiptDto, type ImportEquipmentReceiptItemDto, type EquipmentReceiptPreviewResponseDto } from '../../services/equipmentReceiptService';

interface ImportEquipmentReceiptModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function ImportEquipmentReceiptModal({ isOpen, onClose, onSuccess }: ImportEquipmentReceiptModalProps) {
  const [step, setStep] = useState<'upload' | 'preview' | 'importing'>('upload');
  const [receiptDate, setReceiptDate] = useState(new Date().toISOString().split('T')[0]);
  const [notes, setNotes] = useState('');
  const [createdBy, setCreatedBy] = useState('');
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [parsedItems, setParsedItems] = useState<ImportEquipmentReceiptItemDto[]>([]);
  const [preview, setPreview] = useState<EquipmentReceiptPreviewResponseDto | null>(null);
  ...
  const parseExcelFile = (file: File): Promise<ImportEquipmentReceiptItemDto[]> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const items: ImportEquipmentReceiptItemDto[] = jsonData.map((row, index) => {
            // Support multiple column names (Vietnamese and English)
            ...
            return {
              equipmentCode: String(getCell(['EquipmentCode', 'Equipment Code', 'Mã thiết bị', 'Ma thiet bi', 'Code']) || `EQ-${index + 1}`),
              equipmentName: String(getCell(['EquipmentName', 'Equipment Name', 'Tên thiết bị', 'Ten thiet bi', 'Name']) || 'Unknown'),
              categoryName: getCell(['Category', 'CategoryName', 'Danh mục', 'Danh muc']),
              quantity: parseFloat(getCell(['Quantity', 'Số lượng', 'So luong', 'Qty']) || 1),
              location: getCell(['Location', 'Vị trí', 'Vi tri', 'Position']),
              manufacturer: getCell(['Manufacturer', 'Nhà sản xuất', 'Nha san xuat', 'Brand', 'Maker']),
              model: getCell(['Model', 'Mô hình', 'Mo hinh']),
              serialNumber: getCell(['SerialNumber', 'Serial Number', 'Serial', 'Số serial', 'So serial', 'SN']),
              solasReference: getCell(['SolasReference', 'SOLAS Reference', 'SOLAS', 'Tham chiếu SOLAS', 'Tham chieu SOLAS']),
              specification: getCell(['Specification', 'Spec', 'Thông số', 'Thong so', 'Specs']),
              description: getCell(['Description', 'Mô tả', 'Mo ta', 'Desc']),
              notes: getCell(['Notes', 'Ghi chú', 'Ghi chu', 'Note']),
              lineNumber: index + 1
            };
  ...
  const handlePreview = async () => {
    try {
      ...
      const dto: ImportEquipmentReceiptDto = {
        receiptDate,
      ...
      const previewResult = await equipmentReceiptService.previewImport(dto);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      await equipmentReceiptService.importReceipt(dto);
      onSuccess();
      handleClose();
    } catch (err: any) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Import Phiếu Nhập Thiết Bị</h2>
              <p className="text-sm text-gray-600">Nhập nhiều thiết bị cùng lúc từ dữ liệu Excel</p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
                  ...
                  <li className="ml-6"><code className="bg-blue-100 px-1 rounded">EquipmentCode, EquipmentName, Category, Quantity, Location</code></li>
                  <li className="ml-6">Các cột tùy chọn: <code className="bg-blue-100 px-1 rounded">Manufacturer, Model, SerialNumber, SolasReference, Specification</code></li>
                ...
                <div className="space-y-4">
                  ...
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                      <h4 className="font-semibold text-gray-900">Danh sách Equipment đã đọc từ file</h4>
                    </div>
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-50 sticky top-0">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Mã thiết bị</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Tên thiết bị</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Danh mục</th>
                            <th className="px-3 py-2 text-center text-xs font-medium text-gray-500">Số lượng</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Vị trí</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Nhà SX</th>
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {parsedItems.map((item, idx) => (
                            <tr key={idx} className="hover:bg-gray-50">
                              ...
                              <td className="px-3 py-2 text-sm font-mono">{item.equipmentCode}</td>
                              <td className="px-3 py-2 text-sm">{item.equipmentName}</td>
                              <td className="px-3 py-2 text-sm text-gray-600">{item.categoryName || '-'}</td>
                              <td className="px-3 py-2 text-sm text-center font-medium">{item.quantity}</td>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Equipment Code</th>
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Equipment Name</th>
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Category</th>
                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">Quantity</th>
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Quantity Change</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {preview.items.map((item, idx) => (
                        ...
                        `}>
                          ...
                          <td className="px-3 py-2 text-sm">
                            ...
                            {item.action === 'CREATE' && (
                              <span className="text-xs text-green-600">New: {item.newQuantity}</span>
                            )}
                            {item.errorMessage && (
