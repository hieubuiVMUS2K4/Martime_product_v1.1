...
import { X, Package, Calendar, FileText, Clock, Wrench } from 'lucide-react';
import { equipmentReceiptService } from '../../services/equipmentReceiptService';
import type { EquipmentReceiptResponseDto } from '../../services/equipmentReceiptService';

interface EquipmentReceiptDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  receiptId: number;
}

export function EquipmentReceiptDetailModal({ isOpen, onClose, receiptId }: EquipmentReceiptDetailModalProps) {
  const [receipt, setReceipt] = useState<EquipmentReceiptResponseDto | null>(null);
  ...
  const loadReceiptDetail = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await equipmentReceiptService.getReceiptById(receiptId);
      setReceipt(data);
    } catch (err: any) {
        ...
        <div className="inline-block w-full max-w-5xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-lg shadow-xl">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-blue-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900">Equipment Receipt Details</h3>
                {receipt && (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <InfoCard
                    ...
                    label="Total Items"
                    value={`${receipt.items?.length || 0} items`}
                  />
                  <InfoCard
                ...
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    Equipment Items ({receipt.items?.length || 0})
                  </h4>
                      ...
                      {receipt.items?.map((item, idx) => (
                          <tr key={item.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm border-r border-gray-300">
                              <span className="font-mono text-gray-900">{item.equipmentCode}</span>
                            </td>
                            <td className="px-4 py-3 text-sm border-r border-gray-300">
                              <div className="font-medium text-gray-900">{item.equipmentName}</div>
                              {item.model && (
                                <div className="text-xs text-gray-500 mt-1">Model: {item.model}</div>
                              )}
                              {item.serialNumber && (
