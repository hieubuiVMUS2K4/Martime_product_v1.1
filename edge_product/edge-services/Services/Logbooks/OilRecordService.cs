...
namespace MaritimeEdge.Services.Logbooks
{
    public class OilRecordService : IOilRecordService
    {
        ...
        public OilRecordService(EdgeDbContext context, ILogger<OilRecordService> logger, IVoyageContextService voyageContext)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<(bool Success, Guid? Id, string? Error)> CreateEntryAsync(CreateOilRecordEntryDto dto, string? username = null)
        {
            try
            {
                ...
                var (voyageId, legId) = await _voyageContext.ResolveActiveVoyageAsync(entry.EntryDate);
