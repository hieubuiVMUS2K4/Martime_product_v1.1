                    ...
                    {paginatedLocations.length === 0 ? (
                      ...
                      paginatedLocations.map((loc, idx) => (
                        <tr key={loc.id} className={`hover:bg-blue-50 ${selectedRows.has(loc.id) ? 'bg-blue-50' : idx % 2 === 1 ? 'bg-gray-50/50' : 'bg-white'}`}>
                          ...
                          <td className="px-3 py-2 border-r border-gray-100"><button onClick={() => { if (editMode) selectEditLocation(loc); else { setSelectedNodeId(loc.id); } }} className="flex items-center gap-1 text-blue-600 hover:underline font-medium text-xs text-left w-full"><span className="marquee-cell flex-1 min-w-0"><span className="marquee-text">{loc.name} (SL:{locations.filter(l => l.parentId === loc.id).length})</span></span></button></td>
