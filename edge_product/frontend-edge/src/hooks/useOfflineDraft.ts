 ...
 * Periodically saves the form state to IndexedDB to protect against VSAT dropouts/power cuts.
 * 
 * @param key Unique key for the draft, e.g. 'noon-report'
...
export function useOfflineDraft<T>(
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      ...
      // Basic check: only save if state is not entirely empty or default
      if (current && Object.keys(current).length > 0) {
        try {
          ...
          console.error('Offline auto-save error:', err);
        }
      }
    }, 5000);
  ...
  const restoreDraft = useCallback(() => {
    if (restoredDraft && onRestore) {
      ...
      setHasDraft(false); // Reset prompt
    }
